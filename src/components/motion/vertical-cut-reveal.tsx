 ...
 * VerticalCutReveal
 * -----------------
 * Adapted from 21st.dev — "Vertical Cut Reveal" by @cnippet.dev
 *   https://21st.dev/@cnippet.dev/components/vertical-cut-reveal
 *
 * Changes from the original:
 *   • imports from "framer-motion" instead of "motion/react"
 *   • added a prefers-reduced-motion short-circuit (renders static text)
 * Used for the homepage hero headline (line-by-line clip reveal).
 */
...
>(
  (
    ...
    // Reduced motion: render the words plainly, no clip animation.
    if (reduced) {
      return (
        <span
          className={cn(containerClassName, "block whitespace-pre-wrap")}
          onClick={onClick}
        >
          {text}
        </span>
